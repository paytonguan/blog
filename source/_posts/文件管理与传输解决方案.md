...
全球各大网盘服务之间文件同步工具。免费版每个云服务器只能登陆 1 个账号，不支持计划任务，不支持限速。
...
Windows 安装以上仓库的安装包，iOS 使用以下两个捷径。
...
运行捷径前需要配置。在 Windows 下查看本机 IP，可通过 cmd 下输入 ipconfig 查看。然后在 iOS 下打开捷径的配置，按照提示添加键和值，其中键是 Wi-Fi 名称，值是刚才获取的 IP，其它保持默认即可。

## FnSync

用于 Android 和 Windows 之间共享剪贴板，仅支持 Windows 10。

```
https://gitee.com/holmium/fnsync
```

Windows 安装以上仓库的安装包，Android 安装以下 APP。

```
https://www.coolapk.com/apk/269031
```

完成配置后，对于 Android 10 以下版本即可使用。对于 Android 10 及以上版本，由于 Android 10 对系统权限做了限制，不允许应用后台获取剪贴板，因此需要手动下拉通知栏，并点击发送剪切板文本。
...
把 cue 文件拖入千千静听中，选中你需要转换的歌曲，右键选择`转换格式`即可。
...
手机端可使用 Microsoft Office、Adobe Scan（国区没有此 APP）。电脑端可使用以下软件。
...
打开后选择扫描和 OCR，输出选择`可搜索的图像（精确）`即可。
...
在 Windows 上可通过在命令行输入以下命令，无限续期 Beyond Compare。若无效，则需要删除安装目录下的 BCUnrar.dll 文件。
