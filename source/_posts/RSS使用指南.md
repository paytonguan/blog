---
title: RSS 使用指南
categories: Skill
abbrlink: Instructions-Of-RSS
date: 2020-05-09 10:54:29
tags:
---

![](topic.jpg)

RSS 使用指南。

<!-- more -->

# RSSHub

```
https://github.com/DIYgod/RSSHub
```

## 现成网站

```
http://rss.mk/
https://rss-source.com/
https://getpodcast.xyz/
https://trello.com/b/lvMGhlNB/%F0%9F%8E%89rss-%E6%BA%90%E5%85%B1%E4%BA%AB
https://rss.mifaw.com/
https://blackwinmin.github.io/posts/law_rss/
https://github.com/zhengjim/Chinese-Security-RSS/
https://github.com/Gracker/Rss-IT
https://shimo.im/docs/iwRFK7VNmZIxnuL7/read
http://rsshub.rssforever.com/
https://rss.qiuyuair.com/
http://rsshub.sksren.com/
https://rss.injahow.cn/
https://rss.itggg.cn/
https://rsshub.uneasy.win/
https://rss.shab.fun/
https://rsshub-now.vercel.app/
https://rsshub.vercel.app/
https://hub.slarker.me/
https://rsshub.qufy.me/
https://rsshub.cry33.com/
https://rsshub.anyant.xyz/
http://i.scnu.edu.cn/sub
```

## 手动生成

可用以下链接自建 RSShub。

```
https://docs.rsshub.app/
https://github.com/DIYgod/RSSHub-Radar/
https://greasyfork.org/zh-CN/scripts/373252-rss-show-site-all-rss
```

注意，获得的 RSS 链接在国内不可用，因为`rsshub.app`域名已经被封禁。对于 RSSHub Rader，可以通过进入设置-自定义域名，使用其它域名替换官方域名以绕过封锁。

可选择的域名列表如下。

```
https://uneasy.win/rsshubserver/
https://datatube.dev/cn
```

也可通过腾讯云自行搭建。注册腾讯云并进入后台，选择产品-基础-云开发，选择免费开通，新建环境并点击更多，可以找到 RSSHub 的一键搭建服务。搭建完成后即可获取域名。

## 过滤

### 过滤规则

可在订阅链接中添加参数实现过滤，示例如下。

```
# 只关注V2EX论坛上面有关笔记和Apple的相关讨论，过滤时不区分大小写
# 用|过滤多个关键词，用&组合多个过滤参数
https://rsshub.app/v2ex/topics/latest?filter=笔记|Apple&filter_case_sensitive=false
```

参数列表可查看以下链接。

```
https://docs.rsshub.app/parameter.html
```

### 过滤服务

```
https://synk.info/
https://rssfilter.netlify.app/
https://grepfeed.sigwait.tk/
https://siftrss.com/
```

### 带过滤功能的 RSS 阅读器

```
# Fluent Reader
https://hyliu.me/fluent-reader/

# Feedbro
https://nodetics.com/feedbro/
# 汉化补丁可点击User interface language-Import Locale，复制以下链接即可
https://gist.githubusercontent.com/moelody/3159316ce726fc629fae15278bbce429/raw/aac8c27a5e4cd9a1d44113c182c9cd1f58c54879/feedbro-locale-zh_CN.json
```

# 在线 RSS 服务

## Inoreader

需要翻墙。

```
https://www.innoreader.com/
```

下载 APP 后可通过换源的方式，避免翻墙。在 APP 的设置页面摇一摇手机，将`www.inoreader.com`换为`www.innoreader.com`即可。

## 其它

```
# The Old reader
https://theoldreader.com/

# Qi Reader
https://www.qireader.com.cn/
https://www.qireader.com/
```

# 微信公众号订阅源

```
https://feeddd.org/
https://wechat2rss.xlab.app/
```

# RSS 资源大全

```
https://github.com/AboutRSS/ALL-about-RSS
```
